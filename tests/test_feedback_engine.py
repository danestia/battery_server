...
class TestCalculateDailyIndividualFeedback:
    ...
    def test_perfect_adherence_high_solar(
        ...
        assert device_data["feedback_text"] == f"Date: {date_str}, Score: 100.0%. "
    ...
    def test_worst_adherence_high_solar(
        ...
        assert device_data["net_points_earned"] == -10.0
        assert device_data["positive_points"] == 0.0
        assert device_data["negative_points"] == 10.0
        assert device_data["good_action_hours"] == 0
        assert device_data["bad_action_hours"] == 10
        assert device_data["total_plugged_hours"] == 0.0
        assert device_data["feedback_text"] == f"Date: {date_str}, Score: 0.0%. "
    ...
    def test_mixed_behavior_and_partial_charging(
        ...
        device_data = result["devices"][0]
        assert device_data["device_id"] == "dev1"
        assert device_data["score_pct"] == 94.8
        assert device_data["net_points_earned"] == 8.6
        assert device_data["positive_points"] == 8.6
        assert device_data["negative_points"] == 0.0
        assert device_data["good_action_hours"] == 10
        assert device_data["bad_action_hours"] == 0
        assert device_data["total_plugged_hours"] == 0.5
        assert device_data["feedback_text"] == f"Date: {date_str}, Score: 94.8%. "
    ...
    def test_multiple_devices_scored_independently(
        ...
        dev1_data = next(d for d in result["devices"] if d["device_id"] == "dev1")
        assert dev1_data["score_pct"] == 10.0
        assert dev1_data["net_points_earned"] == -8.0
        assert dev1_data["positive_points"] == 1.0
        assert dev1_data["negative_points"] == 9.0
        assert dev1_data["good_action_hours"] == 1
        assert dev1_data["bad_action_hours"] == 9
        assert dev1_data["total_plugged_hours"] == 1.0

        dev2_data = next(d for d in result["devices"] if d["device_id"] == "dev2")
        assert dev2_data["score_pct"] == 10.0
        assert dev2_data["net_points_earned"] == -8.0
        assert dev2_data["positive_points"] == 1.0
        assert dev2_data["negative_points"] == 9.0
        assert dev2_data["good_action_hours"] == 1
        assert dev2_data["bad_action_hours"] == 9
        assert dev2_data["total_plugged_hours"] == 1.0